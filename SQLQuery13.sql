...
-- 6.  Display Rno, Name, Branch and SPI of all students. 
    ...
    SELECT S.RNO,NAME,BRANCH,SPI FROM STUDENT_INFO S
    LEFT JOIN RESULT R
    ON S.RNO = R.RNO
...
-- 11. Display average result of each branch. 
    ...
    SELECT BRANCH,AVG(SPI)  FROM STUDENT_INFO S
    JOIN RESULT R
    ON S.RNO = R.RNO
    GROUP BY BRANCH
...
-- 13. Display maximum and minimum SPI of each branch. 
    ...
    SELECT BRANCH,MAX(SPI),MIN(SPI) FROM STUDENT_INFO S
    JOIN RESULT R
    ON S.RNO = R.RNO
    GROUP BY BRANCH
...
-- 14. Display branch-wise student count in descending order. 
    ...
    SELECT BRANCH,COUNT(S.RNO) FROM STUDENT_INFO S
    JOIN RESULT R
    ON S.RNO = R.RNO
    GROUP BY BRANCH
...
-- 15. Display branch-wise total SPI of students.
    ...
    SELECT BRANCH,SUM(SPI) FROM STUDENT_INFO S
    JOIN RESULT R
    ON S.RNO = R.RNO
    GROUP BY BRANCH
    ORDER BY BRANCH DESC
...
-- 20. Display branches where maximum SPI is greater than 9. 
     ...
     SELECT BRANCH,MAX(SPI) FROM STUDENT_INFO S
    JOIN RESULT R
    ON S.RNO = R.RNO
    GROUP BY BRANCH
    HAVING MAX(SPI) > 9
