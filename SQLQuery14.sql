...
-- 4.  Find all persons with their department code and location. 
    ...
    SELECT PERSONNAME,DEPARTMENTNAME,LOCATION FROM PERSON P
    JOIN DEPARTMENT D
    ON P.DEPARTMENTID = D.DEPARTMENTID
...
-- 8.  Retrieve person name, salary & department name who belongs to Jamnagar city.
    ...
    SELECT PERSONNAME,DEPARTMENTNAME,SALARY FROM PERSON P
    JOIN DEPARTMENT D
    ON P.DEPARTMENTID = D.DEPARTMENTID
    WHERE CITY='JAMNAGAR'
...
-- 9.  Retrieve person’s detail who joined the Civil department after 1-Aug-2001. 
    ...
    LEFT JOIN DEPARTMENT D
    ON P.DEPARTMENTID = D.DEPARTMENTID
    WHERE DEPARTMENTNAME='CIVIL' AND JOININGDATE > '2001-08-001'
...
-- 12. Give department wise maximum & minimum salary with department name. 
    ...
    SELECT DEPARTMENTNAME,MAX(SALARY) AS MAX,MIN(SALARY) AS MIN FROM PERSON P
    JOIN DEPARTMENT D
    ON P.DEPARTMENTID = D.DEPARTMENTID
    GROUP BY DEPARTMENTNAME
...
-- 13. Find city wise total, average, maximum and minimum salary. 
    ...
    SELECT CITY,AVG(SALARY) AS AVG,MAX(SALARY) AS MAX , MIN(SALARY) AS MIN FROM PERSON P
    JOIN DEPARTMENT D
    ON P.DEPARTMENTID = D.DEPARTMENTID
    GROUP BY CITY
...
-- single column) 
    ...
    SELECT PERSONNAME +'LIVES IN' + CITY + 'AND WORK IN' +DepartmentName + 'DEPARTMENT'   FROM PERSON P
    JOIN DEPARTMENT D
    ON P.DEPARTMENTID = D.DEPARTMENTID
...
-- single column) 
    ...
    SELECT PERSONNAME + 'EARNS' + CAST(SALARY AS VARCHAR(30)) + 'FROM' + DEPARTMENTNAME + 'DEPARTMENT MONTHLY'  FROM PERSON P
    JOIN DEPARTMENT D
    ON P.DEPARTMENTID = D.DEPARTMENTID

-- 17. Find city & department wise total, average & maximum salaries.

    SELECT DEPARTMENTNAME,CITY,AVG(SALARY),MAX(SALARY)  FROM PERSON P
    JOIN DEPARTMENT D
    ON P.DEPARTMENTID = D.DEPARTMENTID
...
-- 19. Find all departments whose total salary is exceeding 100000. 
    ...
    SELECT DEPARTMENTNAME,SUM(SALARY) FROM PERSON P
    JOIN DEPARTMENT D
    ON P.DEPARTMENTID = D.DEPARTMENTID
    GROUP BY DEPARTMENTNAME
